...
(function () {
  ...
  function loadSymbol(symbol, index) {
    ...
    getOptionChain(symbol, '2020-02-10', '2020-02-10', index);
  }
    ...
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      if (json.status == "SUCCESS") {
        ...
        for (shortStrike in callChain) {
          ...
          let shortOption = callChain[shortStrike][0];
          let shortBid = shortOption.bid;
          let shortAsk = shortOption.ask;
          let shortSpread = shortAsk - shortBid;
          let shortMid = (shortBid + shortAsk) / 2;
          ...
          if (shortOption.bid > 0 && fShortStrike > last) {
            ...
            for (longStrike in callChain) {
              let fLongStrike = parseFloat(longStrike);
              let longOption = callChain[longStrike][0];
              let longBid = longOption.bid;
              let longAsk = longOption.ask;
              let longSpread = longAsk - longBid;
              let longMid = (longBid + longAsk) / 2;
              let credit = shortMid - longMid;
              ...
              if (longOption.bid > 0
                ...
                let shortOpenInt = shortOption.openInterest;
                let shortVolm = shortOption.totalVolume;
                let longOpenInt = longOption.openInterest;
                let longVolm = longOption.totalVolume;
                let liq = ((shortBid != 0 && longBid != 0
                  ? Math.max(1 - (
